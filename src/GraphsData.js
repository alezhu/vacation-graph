/**
 * @field {Map<number,GraphData>} map
 */
class GraphsData {
  /**
  ...
  *[Symbol.iterator]() {
  ...
  getChanges(other) {
    ...
    for (const yearData of this) {
      let update = other.map.get(yearData.year);
      if (update) {
        //Update year
        const item = new GraphChanges({ year: yearData.year, usersChanges: yearData.getChanges(update) });
        result.push(item);
      } else {
        //Build delete all changes for year
        const item = new GraphChanges({ year: yearData.year, fullDelete: true });
        result.push(item);
      }
    }
    ...
    for(const yearData of other) {
      if(!this.map.has(yearData.year)) {
        //Build new year
        const item = new GraphChanges({ year: yearData.year, usersChanges: yearData.getChanges(new GraphData()) });
        result.push(item);
      }
    }
