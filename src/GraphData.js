    ...
    *
    ...
    getChanges(other) {
        ...
        for (const userData of this.usersData) {
            const update = other.usersData.get(userData.fio);
            if (update) {
                const item = new GraphUserChanges({
                    fio: userData.fio,
                    periodChanges: userData.getChanges(update)
                });
                result.push(item);
            } else {
        ...
        for (const userData of other) {
            if (!this.usersData.map.has(userData.fio)) {
                ...
                const item = new GraphUserChanges({
                    fio: userData.fio,
                    periodChanges: Periods.getChanges([], userData.periods)
                });
                result.push(item);
            }
        }
